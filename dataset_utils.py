...
def separate_data(data, num_clients, num_labels, niid=False, real=True, partition=None, 
    ...
    if partition == None or partition == "noise":
        ...
        # remain to be tested
        #idx_map = []
        idx_map = [[] for _ in range(num_clients)]
        min_size = 0
        min_require_size = 64
        print("----------------diri alpha 0.1------------------")       
        ...
        for client in range(num_clients):
            ...
            #print("idx_batch[{}] = ".format(client), idx_batch[client])
            print("len(idx_batch[{}]) = ".format(client), len(idx_batch[client]))
            #print("idx_map[{}] = ".format(client), idx_map[client])
